...
function get_twitter_count($url)
{
	...
	$count = isset($array['count']) ? strval($array['count']) : '0';
	$count = ($count);
	return $count;
}
...
function my_reading_time()
{
	...
	$mins = round($count / $per_min) > 0 ? round($count / $per_min) . '分' : null;
	$secs = round($count % $per_min / ($per_min / 60));

	echo <<<EOM

この記事は<span class="b">約{$mins}{$secs}秒</span>で読めます。
...
function change_the_content($the_content)
{
	...
	$the_content = str_replace('このページは会員限定です。登録またはログインをお願いいたします。', '<i class="fa-solid fa-lock"></i>この記事は会員限定です。登録すると続きをお読みいただけます。', $the_content);
	...
	$the_content = str_replace('<a href="https://kakemochi.co.jp/member_edit/?a=pwdreset">パスワードリセット</a>', '', $the_content);
	//会員情報編集ページのみ適用対象
